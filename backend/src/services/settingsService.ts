...
type AssessmentStage = (typeof VALID_ASSESSMENT_STAGES)[number];
  ...
  for (const [key, value] of Object.entries(input)) {
    ...
    if (existing) {
      ...
      const { data: created, error } = await supabaseAdmin
        ...
        .single();
      if (error) {
        throw new AppError(`Failed to create setting: ${key}`, 500);
      }
...
export async function setCurrentSession(
  ...
  if (clearSessionError) {
    throw new AppError('Failed to clear current session', 500);
  }
  ...
  if (error || !updatedSession) {
    throw new AppError('Failed to set current session', 500);
  }
  await logAudit(updatedBy, {
...
export async function setCurrentTerm(
  ...
  const sessionIsCurrent = Array.isArray(term.academic_sessions)
    ? term.academic_sessions[0]?.is_current
    : term.academic_sessions?.is_current;
  if (!sessionIsCurrent) {
    throw new AppError(
      'Only a term belonging to the current academic session can be selected',
      400
    );
  }
...
export async function setAssessmentStage(
  ...
  if (!VALID_ASSESSMENT_STAGES.includes(stage as AssessmentStage)) {
    throw new AppError(
      `Invalid assessment stage. Allowed stages: ${VALID_ASSESSMENT_STAGES.join(', ')}`,
      400
    );
  }
  const { data: term } = await supabaseAdmin
  ...
  const { data: updated, error } = await supabaseAdmin
    .from('terms')
    .update({
      assessment_stage: stage as AssessmentStage,
    })
    .eq('id', termId)
    .select()
    .single();
  ...
  await logAudit(updatedBy, {
    ...
    entity_id: termId,
    metadata: {
      old_stage: term.assessment_stage,
      new_stage: stage,
    },
  });
  return updated;
...
export async function deleteGradingRule(
  ruleId: string,
  deletedBy: string
) {
